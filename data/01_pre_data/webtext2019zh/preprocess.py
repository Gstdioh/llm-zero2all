...
data_dir = "./"
save_dir = "./txt_webtext2019zh"
file_prefix = "txt_webtext2019zh_"
# file_text_len = 2e6  # 中文，大概5.1MB, *40=200MB
file_text_len = 80e6
os.makedirs(save_dir, exist_ok=True)

# 获取当前文件夹下的所有文，除了valid数据集
filenames = sorted(glob.glob(os.path.join(data_dir, "*.json")))
filenames = [filename for filename in filenames if not filename.endswith("valid.json") and not filename.endswith("testa.json")]
...
for filename in filenames:
    with open(filename, "r", encoding="utf-8") as f:
        ...
        for line in lines:
            ...
            # 防止字段不存在
            try:
                texts.append(data["title"] + data["desc"] + data["content"] + "\n")
            except KeyError:
                continue
